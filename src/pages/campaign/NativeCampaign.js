...
import { createNativeCampaign, getCategoryList, getCountryList, getCPCInfo, uploadImage } from '../../app/api';
...
export default function NativeCampaign ({clkFunc}) {
...
const [imgType, setImgType] = useState(0);
  ...
  const showToast = (type) => {
    ...
    if(type == 1) {
      ...
      toast.error("Please upload all required image", {
        position: "top-right",
        autoClose: true,
        hideProgressBar: true,
        closeOnClick: true,
      ...
      if(txt.countries != '' || conchk == true) { 
        ...
        if(txt.device_type.length > 0 && txt.device_os.length > 0) {
          ...
          if((data.pricing_model === 'CPC' && parseFloat(data.cpc_amt) >= parseFloat(cpcost)) || data.pricing_model === 'CPM') {
            ...
            const res = await createNativeCampaign(data)
