...
export const SessionDoughnut = ({ state, className }) => {
  ...
  const data = {
    ...
    datasets: [
      {
        borderColor: "#fff",
        backgroundColor: ["#EB2F48", "#ff6e00", "#FEC30D"],
        data: [state.desktop.percent, state.mobile.percent, state.tablet.percent],
      },
    ],
  };
...
export const SessionOsDoughnut = ({ state, className }) => {
  
  const data = {
    labels: ["Windows", "Apple", "Linux", "Android"],
    dataUnit: "People",
    legend: false,
    datasets: [
      {
        borderColor: "#fff",
        backgroundColor: ["#0377D0", "#000000", "#8f9aaa", "#3BD482"],
        data: [state.windows.percent, state.apple.percent, state.linux.percent,state.android.percent],
      },
    ],
  };
