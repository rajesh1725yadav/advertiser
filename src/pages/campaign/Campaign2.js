...
export const Campaign = () => {
  ...
  const updateAllRows = async (type) => {
    ...
    if(chk.length > 0) {
      ...
      let adt = getAdType();
      let sts = getAdStatus();
      getCampData(adt, sts, currentPage, itemPerPage);
  ...
  const paginate = (pageNumber) => {
    ...
    if(currentPage !== pageNumber) {
      let adt = getAdType();
      let sts = getAdStatus()
      getCampData(adt, sts, pageNumber)
    }
  }

  const { errors, register, handleSubmit } = useForm();

  const getCampData = async (type = '', sts = '', pg=1, lim=0) => {
    setLoading(true)
    let stat = (sts > 0) ? sts : (sts == 0) ? '' : getAdStatus(sts);
    let itemLim = (lim > 0) ? lim : itemPerPage;
    const res = await CampaignList(type, stat, pg, itemLim);
    if(res.data) {
      setData(res.data)
      setPgs(res.row)
      // console.log(res);
                                      ...
                                      <li onClick={() => {  }}  >
                                        <DropdownItem
                                          ...
                                          onClick={(ev) => {
                                            ...
                                            setItemPerPage(100)
                                            let adt = getAdType();
                                            getCampData(adt, adStatus, 1 , 100)
                                          }}
                                        >
                                          <span>100</span>
                                        </DropdownItem>
                                      ...
                                      <li onClick={() => {  }}  >
                                        <DropdownItem
                                          ...
                                          onClick={(ev) => {
                                            ...
                                            setItemPerPage(500)
                                            let adt = getAdType();
                                            getCampData(adt, adStatus, 1 , 500)
                                          }}
                                        >
                                          <span>500</span>
                                        </DropdownItem>
                  ...
                  <TabPane tabId="1">
                    ...
                    <table className="sm-tbl">  
                      ...
                      <tbody>    
                          ...
                          {cinfo.destination_url && <tr><th>Destination URL :</th><td colSpan={3}>{cinfo.destination_url}</td></tr>}
