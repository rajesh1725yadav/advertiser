...
import { createBannerCampaign, createCampaign, getCategoryList, getCountryList, getCPCInfo, uploadImage } from '../../app/api';
...
export default function BannerCampaign ({clkFunc}) {
...
const [imgType, setImgType] = useState(0);
  ...
  if(csUrl.length == 0 || csUrl.indexOf(url) >= 0) {
    if(txt.countries != '' || conchk == true) { 
      ...
      if(resImg.ad1 != '' && resImg.ad2 != '' && resImg.ad3 != '' ) {
      ...
      if(txt.device_type.length > 0 && txt.device_os.length > 0) {
        ...
        if((data.pricing_model === 'CPC' && parseFloat(data.cpc_amt) >= parseFloat(cpcost)) || data.pricing_model === 'CPM') {
          ...
          const res = await createBannerCampaign(data)
    ...
    useEffect(()=>{
      ...
      console.log(campState.sts.device_os);
      // setConChk(false)
        ...
        <Col md="7">
          <FormGroup>
            ...
            <input
              ...
              ref={register({ required: true, pattern : /(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,})/ })}
            ...
            <input
              ...
              ref={register({ pattern : /(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,})/})}
                ...
                <div className="custom-control custom-checkbox custom-control-pro custom-control-pro-icon no-control checked">
                  ...
                  onChange={(e)=> {
                    ...
                    if(e.target.checked == true) {
                      ...
                      setTxt({ ...txt, device_type: dtype, device_os: dos })
                      } else {
                        let data = dtype.filter(item => item !== e.target.value)
                        
                        let data2 = dos.filter(item => item !== 'linux')
                        data2 = data2.filter(item => item !== 'windows')
                        // setTxt({ ...txt, device_os: data2 })
                        setTxt({ ...txt, device_type: data, device_os: data2 })
                      }
                    }} 
          ...
          <div className="form-group">
              ...
              <div className='form-group'> 
                <div className="mt-2 sm-drop-zone2">
                  <img src={(resImg.ad2 != '') ? resImg.ad2 : AdView2} />
                </div>
                <a href='javascript:;' className='btn btn-primary mt-2'>
                  <input type="file" className='sm-input' onChange={(e) => {
                    setImgRatio(4/3);
              ...
              <div className='form-group'> 
                <div className="mt-2 sm-drop-zone3">
                  <img src={(resImg.ad3 != '') ? resImg.ad3 : AdView3} />
                </div>
                <a href='javascript:;' className='btn btn-primary mt-2'>
                  <input type="file" className='sm-input' onChange={(e) => {
                    setImgRatio(3/1);
